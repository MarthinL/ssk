 ...
 *
 * Algorithm: Iterate through set bit positions in descending order.
 ...
 *   Position 5: rank += C(5,3) = 10
 *   Position 3: rank += C(3,2) = 3
 *   Position 1: rank += C(1,1) = 1
 *   Final rank = 14
 * ============================================================================
 */
...
ssk_combinadic_rank(uint64_t bits, uint8_t n, uint8_t k)
{
    ...
    /* Scan from MSB (position n-1) to LSB (position 0) */
    for (int pos = n - 1; pos >= 0 && remaining_k > 0; pos--)
    {
        if (bits & (1ULL << pos))
        {
 ...
 *
 * Algorithm: Greedily find the highest position p such that C(p, remaining_k) <= remaining_rank.
 * Set that bit, subtract C(p, remaining_k) from rank, decrement remaining_k.
 *
 * Example: rank=14, n=6, k=3
 *   remaining_k=3, remaining_rank=14
 *   Try p=5: C(5,3)=10 <= 14, set bit 5, rank=4, k=2
 *   Try p=4: C(4,2)=6 > 4, skip
 *   Try p=3: C(3,2)=3 <= 4, set bit 3, rank=1, k=1
 *   Try p=2: C(2,1)=2 > 1, skip
 *   Try p=1: C(1,1)=1 <= 1, set bit 1, rank=0, k=0
 *   Result: bits = 0b101010
 * ============================================================================
 */
...
ssk_combinadic_unrank(uint64_t rank, uint8_t n, uint8_t k)
{
    ...
    /* Scan from MSB (position n-1) down to LSB (position 0) */
    for (int pos = n - 1; pos >= 0 && remaining_k > 0; pos--)
    {
