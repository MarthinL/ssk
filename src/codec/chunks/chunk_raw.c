 ...
 *   - run_length encoded with VLQ-P (VLQP_SMALL_INT)
 ...
 *   - As RAW_RUN: 2 + vlqp(3) + 3*64 = ~199 bits (plus run_len saves on many chunks)
...
raw_token_encode(uint64_t bits, uint8_t n, uint8_t *buf, size_t bit_pos)
{
    ...
    bs_write_bits(buf, bit_pos, TOK_RAW, 2);
    bit_pos += 2;
    
    /* 2. Write raw bits */
    bs_write_bits(buf, bit_pos, bits, n);
    bit_pos += n;
                 ...
                 uint8_t n, uint64_t *bits_out, size_t *bits_read)
{
    ...
    *bits_out = bs_read_bits(buf, bit_pos, n);
    *bits_read = n;
...
{
    /* 2 (type) + vlqp(run_len) + run_len*chunk_bits + final_nbits */
    size_t vlqp_bits = vlqp_encoded_bits(run_len, &VLQP_SMALL_INT);
    size_t data_bits = (size_t)run_len * chunk_bits + final_nbits;
    
    return 2 + vlqp_bits + data_bits;
}
...
raw_run_header_encode(uint16_t run_len, uint8_t *buf, size_t bit_pos)
{
    ...
    bs_write_bits(buf, bit_pos, TOK_RAW_RUN, 2);
    bit_pos += 2;
    
    /* 2. Write run length with VLQ-P */
    size_t vlqp_bits = vlqp_encode(run_len, &VLQP_SMALL_INT, buf, bit_pos);
    bit_pos += vlqp_bits;
                      ...
                      uint16_t *run_len_out, size_t *bits_read)
{
    ...
    int rc = vlqp_decode(buf, bit_pos, buf_bits, &VLQP_SMALL_INT, &run_len, &vlqp_read);
    if (rc != 0)
        return -1;
               ...
               uint8_t *dst, size_t dst_bit_pos)
{
    ...
    /* Copy raw data */
    size_t data_bits = (size_t)run_len * chunk_bits + final_nbits;
    bs_copy_bits(src_bits, src_bit_pos, dst, dst_bit_pos, data_bits);
