 ...
 *   [VLQ-P] length_bits (number of bits in this run)
 *
 * MIX Segment Format:
 *   [1 bit] segment_type = 1
 *   [VLQ-P] initial_delta (bit offset from partition start or previous segment end)
 *   [VLQ-P] length_bits (total bits in this segment)
...
{
    /* 1 (type) + 1 (membership_bit) + VLQ-P(length_bits) */
    size_t vlqp_bits = vlqp_encoded_bits(length_bits, spec->vlqp_length_bits);
    return 1 + 1 + vlqp_bits;
}
                   ...
                   const SSKFormatSpec *spec, uint8_t *buf, size_t bit_pos)
{
    ...
    bs_write_bits(buf, bit_pos, SEG_RLE, 1);
    bit_pos += 1;
    
    /* 2. Membership bit (1 bit) */
    bs_write_bits(buf, bit_pos, membership_bit & 1, 1);
    bit_pos += 1;
    
    /* 3. Length in bits (VLQ-P) */
    size_t vlqp_written = vlqp_encode(length_bits, spec->vlqp_length_bits, buf, bit_pos);
    bit_pos += vlqp_written;
...
{
    /* 1 (type) + VLQ-P(initial_delta) + VLQ-P(length_bits) */
    size_t delta_bits = vlqp_encoded_bits(initial_delta, spec->vlqp_initial_delta);
    size_t length_vlqp = vlqp_encoded_bits(length_bits, spec->vlqp_length_bits);
    
    return 1 + delta_bits + length_vlqp;
}
                          ...
                          uint8_t *buf, size_t bit_pos)
{
    ...
    bs_write_bits(buf, bit_pos, SEG_MIX, 1);
    bit_pos += 1;
    
    /* 2. Initial delta (VLQ-P) */
    size_t delta_written = vlqp_encode(initial_delta, spec->vlqp_initial_delta, buf, bit_pos);
    bit_pos += delta_written;
    
    /* 3. Length in bits (VLQ-P) */
    size_t length_written = vlqp_encode(length_bits, spec->vlqp_length_bits, buf, bit_pos);
...
segment_read_type(const uint8_t *buf, size_t bit_pos, size_t buf_bits)
{
    ...
    return (int)bs_read_bits(buf, bit_pos, 1);
}
