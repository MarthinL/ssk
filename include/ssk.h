...
/**
 * Compute bits needed to encode a value with given VLQ-P params.
 *
 * @param value Integer to encode
 * @param params VLQ-P parameters
 * @return Bits needed, or 0 if value exceeds domain
 */
size_t vlqp_encoded_bits(uint64_t value, const VLQPParams *params);

/**
 * Encode an integer using VLQ-P at a bit position.
 *
 * @param value Integer to encode
 * @param params VLQ-P parameters
 * @param buf Output buffer (must have sufficient space)
 * @param bit_pos Starting bit position in buffer
 * @return Bits written, or 0 on error
 */
size_t vlqp_encode(uint64_t value, const VLQPParams *params, uint8_t *buf, size_t bit_pos);

/**
 * Decode a VLQ-P encoded integer from bit position.
 *
 ...
 */
int vlqp_decode(const uint8_t *buf, size_t bit_pos, size_t buf_bits,
                const VLQPParams *params, uint64_t *value_out, size_t *bits_read);

/**
 * Check if a VLQ-P encoding is minimal (canonical).
 * Decoder must reject non-minimal encodings.
 *
 * @param buf Input buffer
 * @param bit_pos Starting bit position
 * @param buf_bits Total bits available
 * @param params VLQ-P parameters
 * @return true if minimal, false if non-minimal
 */
bool vlqp_is_minimal(const uint8_t *buf, size_t bit_pos, size_t buf_bits,
                     const VLQPParams *params);

/**
 * Get maximum value encodable in given number of stages.
 *
 * @param params VLQ-P parameters
 * @param stages Number of stages (1 to n_stages)
 * @return Maximum value for that many stages
 */
uint64_t vlqp_max_value(const VLQPParams *params, uint8_t stages);
...
// ============================================================================
// ORDINALITY CODEC (Legacy - may be superseded by VLQ-P)
// ============================================================================
