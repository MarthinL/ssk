 ...
 *   - VLQ-P: Minimally encoded (no unnecessary continuation bits)
 *
...
// ============================================================================
// VLQ-P (Variable Length Quantity - Parameterized)
// ============================================================================

/**
 * VLQ-P (Variable Length Quantity - Parameterized)
 * 
 * A VLQ-P subtype is defined by:
 *   - domain_size: Maximum value this VLQ-P can encode
 *   - n_stages: Number of extension stages
 *   - stage_bits[]: Array of bit counts for each stage
 * 
 * Encoding process:
 *   1. Start with stage_bits[0] bits
 *   2. If value doesn't fit, add continuation bit and extend with stage_bits[1] bits
 *   3. Continue until value fits or all stages exhausted
 * 
 * Example: stage_bits = {2, 5, 8, 16}
 *   - Values 0-3: 2 bits (no continuation)
 *   - Values 4-131: 2 + 5 bits (1 continuation)
 *   - Values 132-32899: 2 + 5 + 8 bits (2 continuations)
 *   - Values 32900+: 2 + 5 + 8 + 16 bits (3 continuations)
 * 
 * TUNING PHILOSOPHY:
 * The power of VLQ-P lies in matching stage_bits[] to the specific distribution
 * of values for each encoded field. Theoretical optimization is impractical;
 * instead, tune based on empirical data:
 * 
 *   1. Profile real-world SSK encodings (e.g., 1M representative subsets)
 *   2. Analyze value distributions for each field (histograms, percentiles)
 *   3. Design stage_bits[] to minimize total bits for observed distributions
 *   4. Test variants, measure total encoding sizes, iterate
 * 
 * Initial VLQ-P subtypes are educated guesses. Refine after profiling production data.
 * 
 * The "linking problem" (which subtype for which field) is equally empirical:
 * assign based on observed characteristics, measure results, adjust as needed.
 */
typedef struct VLQPParams {
    uint64_t domain_size;      // Maximum encodable value
    uint8_t  n_stages;         // Number of stages (length of stage_bits array)
    uint8_t  stage_bits[16];   // Bit count for each stage
} VLQPParams;

// VLQ-P subtypes (defined in ssk_vlq.c)
// Initial definitions based on educated guesses; tune after profiling
extern const VLQPParams VLQP_DEFAULT;          // For format version (must encode 0 in minimal space)
extern const VLQPParams VLQP_SMALL_INT;        // Expected: mostly 0-10, occasional 100s
extern const VLQPParams VLQP_MEDIUM_INT;       // Expected: mostly 10-1000, occasional 10Ks
extern const VLQPParams VLQP_LARGE_INT;        // Expected: wide spread, 0-2^32, clustered
extern const VLQPParams VLQP_HUGE_INT;         // Expected: rare, but 2^64 must be possible
...
typedef struct SSKFormatSpec {
    ...
    // VLQ-P linking (assigned subtypes for each encoded field)
    // These define which VLQ-P parameters are used for each field type.
    // Once stabilized, these become immutable for the format.
    const VLQPParams *vlqp_format_version;     // For format version field
    const VLQPParams *vlqp_partition_delta;    // For partition gaps
    const VLQPParams *vlqp_segment_count;      // For segment counts
    const VLQPParams *vlqp_initial_delta;      // For segment offset gaps
    const VLQPParams *vlqp_length_bits;        // For segment lengths
    const VLQPParams *vlqp_popcount;           // For popcount encoding
    const VLQPParams *vlqp_enum_combined;      // For (k << rank_bits) | rank
    const VLQPParams *vlqp_raw_run_len;        // For RAW_RUN length
 ...
 * Indexed as ssk_rank_bits[n][k] for n=0..64, k=0..18
 * 
 * Must be initialized at module load via ssk_combinadic_init().
 */
extern uint8_t ssk_rank_bits[65][19];
 ...
 *   - VLQ-P values minimally encoded
...
/**
 * Check if a VLQ-P encoded value is minimal (canonical).
 * 
 * @param encoded_bytes Start of VLQ-P encoded value
 * @param vlqp_params VLQ-P parameters for this field
 * @param value The decoded value
 * @return true if minimally encoded, false if non-canonical
 */
bool ssk_vlqp_is_minimal(const uint8_t *encoded_bytes, 
                        const VLQPParams *vlqp_params,
                        uint64_t value);
