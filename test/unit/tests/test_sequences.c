...
TEST(test_vlqp_encode_small)
{
    uint8_t buf[16] = {0};
    
    /* VLQP_DEFAULT: stage_bits = {4, 4}, n_stages = 2
     * Values 0-15 fit in stage 0: 4 data bits + 1 cont bit = 5 bits
     * Value 16+ needs stage 1: 4 + 1 + 4 = 9 bits (no cont after final stage)
     */
    size_t bits = vlqp_encode(0, &VLQP_DEFAULT, buf, 0);
    ASSERT_EQ(bits, 5);  /* 4 data + 1 cont=0 */
    
    bits = vlqp_encode(15, &VLQP_DEFAULT, buf, 0);
    ASSERT_EQ(bits, 5);  /* 4 data + 1 cont=0 */
    
    /* Value 16 needs both stages: 4 + 1(cont=1) + 4 = 9 bits */
    bits = vlqp_encode(16, &VLQP_DEFAULT, buf, 0);
    ASSERT_EQ(bits, 9);
}

TEST(test_vlqp_roundtrip)
{
    uint8_t buf[16] = {0};
    uint64_t decoded;
    size_t bits_written, bits_read;
    
    /* Test various values with VLQP_SMALL_INT */
    uint64_t test_values[] = {0, 1, 15, 16, 100, 1000, 65535};
    ...
    for (size_t i = 0; i < sizeof(test_values)/sizeof(test_values[0]); i++)
    {
        ...
        bits_written = vlqp_encode(val, &VLQP_SMALL_INT, buf, 0);
        ASSERT(bits_written > 0);
        
        int rc = vlqp_decode(buf, 0, 128, &VLQP_SMALL_INT, &decoded, &bits_read);
        ASSERT_EQ(rc, 0);
...
TEST(test_vlqp_minimality)
{
    ...
    bits_written = vlqp_encode(5, &VLQP_DEFAULT, buf, 0);
    ASSERT(vlqp_is_minimal(buf, 0, 128, &VLQP_DEFAULT));
}

TEST(test_vlqp_large_values)
{
    uint8_t buf[32] = {0};
    uint64_t decoded;
    size_t bits_written, bits_read;
    
    /* Test with VLQP_LARGE_INT for larger values */
    uint64_t test_values[] = {0, 31, 32, 4095, 4096, 1000000, UINT32_MAX};
    ...
    for (size_t i = 0; i < sizeof(test_values)/sizeof(test_values[0]); i++)
    {
        ...
        bits_written = vlqp_encode(val, &VLQP_LARGE_INT, buf, 0);
        ASSERT(bits_written > 0);
        
        int rc = vlqp_decode(buf, 0, 256, &VLQP_LARGE_INT, &decoded, &bits_read);
        ASSERT_EQ(rc, 0);
        ASSERT_EQ(decoded, val);
    }
}
...
/* Full spec for segment tests (needs VLQ-P params) */
static SSKFormatSpec full_test_spec = {
    ...
    .vlqp_length_bits = &VLQP_MEDIUM_INT,
    .vlqp_initial_delta = &VLQP_LARGE_INT,
};
...
/* Extended test spec with partition VLQ-P params */
static SSKFormatSpec partition_test_spec = {
    ...
    .n_bits_for_k = 6,
    .rare_run_threshold = 64,
    .vlqp_format_version = &VLQP_DEFAULT,
    .vlqp_partition_delta = &VLQP_LARGE_INT,
    .vlqp_segment_count = &VLQP_SMALL_INT,
    .vlqp_length_bits = &VLQP_MEDIUM_INT,
    .vlqp_initial_delta = &VLQP_LARGE_INT,
};
...
{
    printf("\n=== VLQ-P Tests ===\n");
    RUN_TEST(test_vlqp_encode_small);
    RUN_TEST(test_vlqp_roundtrip);
    RUN_TEST(test_vlqp_minimality);
    RUN_TEST(test_vlqp_large_values);
