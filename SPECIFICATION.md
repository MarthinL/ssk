...
> The current VLQ-P implementation in the codebase is a development placeholder that does not fully align with the original design intent described below. It is functional for testing and development with disposable data but **MUST NOT** be used in production for Format 0. A proper VLQ-P implementation conforming to this specification will be required before production deployment to ensure canonical bijection and efficiency.
...
An encoded SSK is a sequence of VLQ-P encoded integers and bit fields:

```
[format_version]                    VLQ-P

For each partition (ascending order):
  [partition_delta]                 VLQ-P: (H_i - H_{i-1} - 1)
  [n_segments]                      VLQ-P
  
  For each segment (ascending offset order):
    [seg_kind_tag]                  1 bit: 0=RLE, 1=MIX
    [initial_delta]                 VLQ-P: gap from previous segment
    [length_bits]                   VLQ-P: segment length in bits
    ...
    If MIX:
      [token_stream]                Sequence of tokens (last_chunk_nbits derivable from length_bits):
        ...
        RAW_RUN token:
          [token_tag]               2 bits: 10
          [run_len]                 VLQ-P: number of chunks
          [bits]                    run_len * 64 + optional final bits
        
        ENUM_RUN token:
          [token_tag]               2 bits: 11
          [run_len]                 VLQ-P: number of identical chunks
          [k]                       6 bits: popcount
...
### Rule 8: VLQ-P Minimality

Every VLQ-P encoded value must use the **minimum number of continuation bits** required:
...
**Decoder validation:**
```c
bool is_minimal(vlqp_bytes, value, vlqp_params) {
...
bool validate_canon(encoded_bytes, len, format_spec) {
    ...
    // 3. VLQ-P minimality
    for each vlqp_value:
        if (!is_minimal(value, vlqp_params))
            return false;
...
Every encoded SSK begins with a **format version** encoded as VLQ-P:

```c
[format_version]  VLQ-P using VLQP_DEFAULT parameters
```
...
## VLQ-P: Parameterized Variable-Length Encoding

SSK uses **VLQ-P** (Variable Length Quantity - Parameterized) for all variable-length integer fields.
...
```c
uint64_t vlqp_encode(uint64_t value, const VLQPParams *params) {
    uint64_t result = 0;
    int bit_pos = 0;
    int stage = 0;
...
### VLQ-P Subtypes for Format 0

Each encoded field has a specific VLQ-P subtype, chosen to minimize total encoded size for typical value distributions:

**Field → Subtype assignments** (frozen for Format 0):
- `format_version` → `VLQP_DEFAULT` (must encode 0 in minimal space)
- `partition_delta` → `VLQP_LARGE_INT` (0 frequent, large gaps occasional)
- `segment_count` → `VLQP_SMALL_INT` (typically 1-10)
- `initial_delta` → `VLQP_INITIAL_DELTA` (0 frequent, gaps up to 2^32)
- `length_bits` → `VLQP_MEDIUM_INT` (typically 64-2048)
- `enum_combined` → `VLQP_LARGE_INT` (wide range depending on k and rank)
- `raw_run_len` → `VLQP_SMALL_INT` (typically 2-100)
...
void test_canon_rejection() {
    ...
    uint8_t bad_vlq_minimal[] = {...};       // Non-minimal VLQ-P
    ...
    assert_error(decode(bad_vlq_minimal), ERRCODE_DATA_CORRUPTED);
