...
- [ ] **Task 1:** Implement memory functions (abv_alloc, abv_grow, abv_free)
...
1. **abv_alloc(format_version, initial_size)**
...
2. **abv_grow(abv, needed_bytes)**
   ...
   - Return new AbV (may differ from input)
...
```
AbV ssk_keystore_decode(bytea *ssk);
bytea *ssk_keystore_encode(AbV abV);
```
...
1. **ssk_keystore_decode(bytea *encoded)**
   ...
   - Allocate empty AbV via abv_alloc() with reasonable initial size (~1KB)
...
static void hierarchy_add_value(AbV abv, uint64_t value);
static AbVPartition *hierarchy_get_or_create_partition(AbV abv, uint16_t partition_id);
static AbVSegment *hierarchy_get_or_create_segment(AbVPartition *part, uint16_t segment_id);
```

**Design Details:**

1. **ssk_sfunc(state, value)**
   - If state is NULL: initialize via abv_alloc(0, 4096) (empty SSK for Format 0)
...
4. **hierarchy_get_or_create_partition(dec, partition_id)**
   ...
   - If yes: return pointer to AbVPartition at that offset
   - If no: allocate new AbVPartition in dec's free space (grow if needed)
...
5. **hierarchy_get_or_create_segment(part, segment_id)**
   ...
   - Allocate new AbVSegment if needed
